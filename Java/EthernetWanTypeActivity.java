...
public class EthernetWanTypeActivity extends AppCompatActivity implements OnClickListener, OnCheckedChangeListener {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
                ...
                if (mInfo != null && mInfo.isConnected()){
                    ...
                    String ethernet_mode = EthernetHelper.getConnectModeWithType(context, NetworkAbility.NETWORK_TYPE_WAN);
                    updateAddressGetWay(ethernet_mode);

                    m_EditIP.setText(EthernetHelper.getIpAddress(context, NetworkAbility.NETWORK_TYPE_WAN));
                    m_EditMask.setText(EthernetHelper.getNetMask(context, NetworkAbility.NETWORK_TYPE_WAN));
                    m_EditGateway.setText(EthernetHelper.getGateWay(context, NetworkAbility.NETWORK_TYPE_WAN));
                    m_EditDNS.setText(EthernetHelper.getDns(context, NetworkAbility.NETWORK_TYPE_WAN));
                }else{
        ...
        switch (v.getId()){
            case R.id.btOK:
                if (m_NetType == DHCP_MODE) {
                    ...
                    EthernetHelper.setDhcpConfig(this, NetworkAbility.NETWORK_TYPE_WAN);
                    ...
                    if (ip.length() > 0 && mask.length() > 0 && gateway.length() > 0 && dns.length() > 0) {
                        ...
                        EthernetHelper.setStaticConfig(this, NetworkAbility.NETWORK_TYPE_WAN, ip, mask, gateway, dns);
                    }
                    else {
