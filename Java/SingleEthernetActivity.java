...
public class SingleEthernetActivity extends AppCompatActivity implements OnClickListener, OnCheckedChangeListener {
    ...
    public static int singleEthernetType = -1;
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(ConnectivityManager.CONNECTIVITY_ACTION)) {
                ...
                if (mInfo != null && mInfo.isConnected()){
                    ...
                    String ethernet_mode = EthernetHelper.getConnectModeWithType(context, SingleEthernetActivity.singleEthernetType);
                    updateAddressGetWay(ethernet_mode);

                    m_EditIP.setText(EthernetHelper.getIpAddress(context, SingleEthernetActivity.singleEthernetType));
                    m_EditMask.setText(EthernetHelper.getNetMask(context, SingleEthernetActivity.singleEthernetType));
                    m_EditGateway.setText(EthernetHelper.getGateWay(context, SingleEthernetActivity.singleEthernetType));
                    m_EditDNS.setText(EthernetHelper.getDns(context, SingleEthernetActivity.singleEthernetType));
                }else{
        ...
        switch (v.getId()){
            case R.id.btOK:
                if (m_NetType == DHCP_MODE) {
                    ...
                    EthernetHelper.setDhcpConfig(this);
                    ...
                    if (ip.length() > 0 && mask.length() > 0 && gateway.length() > 0 && dns.length() > 0) {
                        ...
                        EthernetHelper.setStaticConfig(this, ip, mask, gateway, dns);
                    }
                    else {
